...
from emnist import CustomEMNIST
...
def get_loader(args, shuffle = True, use_cuda = True):
    ...
    if args.dataset == "MNIST":
        ...
        train_dataset = CustomEMNIST(
            'KANbeFair/dataset/emnist_new/gzip',
            split="letters",
            train=True,
            transform=transform
        )
        test_dataset = CustomEMNIST(
            'KANbeFair/dataset/emnist_new/gzip',
            split="letters",
            train=False,
            transform=transform
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset,**train_kwargs)
        test_loader = torch.utils.data.DataLoader(test_dataset, **test_kwargs)
        num_classes = 47  # balanced版本有47个类别
        input_size = 1 * 28 * 28
        ...
        train_dataset = datasets.FashionMNIST('KANbeFair/dataset/FashionMNIST', train=True, download=True,
                        transform=transform)
        test_dataset =  datasets.FashionMNIST('KANbeFair/dataset/FashionMNIST', train=False, download=True,
                        transform=transform)
        train_loader = torch.utils.data.DataLoader(train_dataset,**train_kwargs)
        test_loader = torch.utils.data.DataLoader(test_dataset, **test_kwargs)
        num_classes = 10
        ...
        train_dataset = datasets.ImageFolder('../dataset/tiny-imagenet-200/tiny-imagenet-200/train', transform=transform)
        test_dataset = datasets.ImageFolder('../dataset/tiny-imagenet-200/tiny-imagenet-200/tidy_val', transform=transform)
        train_loader = torch.utils.data.DataLoader(train_dataset,**train_kwargs)
        test_loader = torch.utils.data.DataLoader(test_dataset, **test_kwargs)
        num_classes = 200
        input_size = 3 * 28 * 28
...
def get_model(args):
    if args.model == "MLP":
        ...
        model = KAF(args)
    elif args.model == "KAF_Text":
        model = KAF_Text(args)
    elif args.model == "GPKAN":
        model = GPKAN(args)
    elif args.model == "FAN":
        model = FAN(args)
    elif args.model == "FAN_Text":
        model = FAN_Text(args)
    elif args.model == "GPKAN_Text":
        model = GPKAN_Text(args)
    else:
        raise NotImplementedError
    return model
